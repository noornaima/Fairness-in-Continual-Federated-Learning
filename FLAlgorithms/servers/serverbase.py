...
class Server:
    ...
    def select_users(self, round, num_users, return_idx=False):
        ...
        active_users_list = list(self.users)

        # Start with random selection for the first round or if the technique is 'random'
        if round == 0 :
            #active_users_list = [user for user in self.users if user not in self.user_participation_rounds]
            selected_users = random.sample(self.users, num_users)
            print("Selecting users randomly")
        else:
            # Check the fairness ratio before selecting users
            if self.fairness_ratio > self.fairness_threshold:
                print(f"Continuing with {self.current_technique} as fairness ratio is satisfactory.")
            else:
                # Choose the next technique if the fairness ratio is below the threshold
                self.current_technique = self.choose_next_technique(round)
                print(f"Switching to {self.current_technique} due to low fairness ratio.")

            selected_users = self.apply_technique(self.current_technique, num_users)


        # Update participation info for all selected users
        for user in selected_users:
            ...
            print(
                f"User {user_id}: Participation count updated to {self.user_participation_count[user_id]} in round {round}")
    ...
    def choose_next_technique(self, round):
        # Calculate the moving average of the fairness scores for each technique and select the next one
        current_performance = self.technique_performance[self.current_technique][-self.moving_average_window:]
        moving_average = sum(current_performance) / len(current_performance) if current_performance else 0

        if moving_average <= self.fairness_threshold:
            next_index = (self.techniques.index(self.current_technique) + 1) % len(self.techniques)
            self.current_technique = self.techniques[next_index]

        return self.current_technique

    def apply_technique(self, technique, num_users):
        # Select users based on the current technique
        if self.current_technique == 'low participation':
            #selected_users = self.low_part(num_users)
            print("Selecting users based on low participation")
            return self.low_part(num_users)
        elif self.current_technique == 'low accuracy':
            #selected_users = self.low_acc(num_users)
            print("Selecting users based on low accuracy")
            return self.low_acc(num_users)
        elif self.current_technique == 'low average':
            #selected_users = self.low_avg(num_users)
            print("Selecting users based on low average")
            return self.low_avg(num_users)
        #else:
         #   return random.sample(self.users, num_users)

    def low_part(self, num_users):
        # Select users based on lower participation counts
        sorted_users = sorted(self.users, key=lambda user: self.user_participation_count.get(user.id, 0))
        return sorted_users[:num_users]

    def low_acc(self, num_users):
        # Select users with the lowest accuracies from the previous round
        sorted_users = sorted(self.users, key=lambda user: self.total_accuracies.get(user.id, float('inf')))
        return sorted_users[:num_users]

    def low_avg(self, num_users):
        # Select users based on a low average of a feature set
